...
function calcAverageHumanAge(ages) {
    // Step 1: Calculate human age for each dog
    const humanAges = ages.map(dogAge => {
        return dogAge <= 2 ? dogAge * 2 : 16 + dogAge * 4;
    });

    // Step 2: Filter out dogs that are less than 18 human years old
    const adultDogs = humanAges.filter(humanAge => humanAge >= 18);

    // Step 3: Calculate the average human age of remaining dogs
    const averageHumanAge = adultDogs.reduce((sum, age) => sum + age, 0) / adultDogs.length;

    // Return the average age (or 0 if no adult dogs are present)
    return adultDogs.length > 0 ? averageHumanAge : 0;
}
